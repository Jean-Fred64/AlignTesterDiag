...
| `FORMAT_HEAD_SETTLE_MS` | `15 ms` | **Format Track Step Settle Delay:** Optimized mechanical carriage dampening time after single-track stepping during low-level format operations. |
| `FORMAT_HEAD_SWITCH_SETTLE_MS` | `20 ms` | **Format Head Switch Settle Delay:** Preamplifier stabilization delay when switching between Head 0 and Head 1 during format operations on the same cylinder. |
...
| <kbd>F</kbd> / <kbd>f</kbd> | **Low-Level Track & Disk Formatter** | `UI` & `Hardware I/O` | Opens the Low-Level Format Confirmation Modal. Allows dynamic interactive track count adjustment with `+`/`-` or Arrow keys (40 tracks standard `00..39`, max 42 for 48 TPI; 80 tracks standard `00..79`, max 84 for 96/135 TPI). Dispatches `HwCmd::FormatTrack` on <kbd>T</kbd> (formats current track only) or `HwCmd::FormatDisk` on <kbd>D</kbd> (formats entire disk across both heads) using cycle-accurate 72 MHz pulse synthesis, write pre-compensation, physical index cueing, and fast 1-revolution read-after-write CRC verification. |
